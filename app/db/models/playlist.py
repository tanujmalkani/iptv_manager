...
class PlaylistEntry(Base):
    ...
    source_playlist_version_id: Mapped[int] = mapped_column(ForeignKey("source_playlist_versions.id", ondelete="CASCADE"), nullable=False, index=True)
    channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="RESTRICT"), nullable=False, index=True)
    stream_id: Mapped[int] = mapped_column(ForeignKey("streams.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    source_playlist_version: Mapped["SourcePlaylistVersion"] = relationship(back_populates="entries")
