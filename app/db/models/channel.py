...
class Channel(Base):
    ...
    options: Mapped[list["ChannelOption"]] = relationship(back_populates="channel", cascade="all, delete-orphan")
    streams: Mapped[list["ChannelStream"]] = relationship(back_populates="channel", cascade="all, delete-orphan")
    selection: Mapped["ChannelSelection | None"] = relationship(back_populates="channel", uselist=False, cascade="all, delete-orphan")
    source_merges: Mapped[list["ChannelMerge"]] = relationship(foreign_keys="ChannelMerge.source_channel_id", back_populates="source_channel", cascade="all, delete-orphan")
    target_merges: Mapped[list["ChannelMerge"]] = relationship(foreign_keys="ChannelMerge.target_channel_id", back_populates="target_channel", cascade="all, delete-orphan")


class ChannelOption(Base):
    __tablename__ = "channel_options"
    __table_args__ = (UniqueConstraint("channel_id", "option_type", "value", name="uq_channel_option"),)
    ...
    source_playlist_id: Mapped[int | None] = mapped_column(ForeignKey("source_playlists.id", ondelete="SET NULL"))
    playlist_entry_id: Mapped[int | None] = mapped_column(ForeignKey("playlist_entries.id", ondelete="SET NULL"))
...
class ChannelMerge(Base):
    ...
    __table_args__ = (UniqueConstraint("source_channel_id", "target_channel_id", name="uq_channel_merge"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    source_channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"), nullable=False)
    target_channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, nullable=False)

    source_channel: Mapped["Channel"] = relationship(foreign_keys=[source_channel_id], back_populates="source_merges")
    target_channel: Mapped["Channel"] = relationship(foreign_keys=[target_channel_id], back_populates="target_merges")
