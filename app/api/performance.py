...
from app.api.schemas import ChannelPerformanceResponse, ChannelSummaryResponse, StreamPerformanceResponse
from app.db.models import ChannelStream, StreamTest
...
def get_stream_performance_endpoint(
    ...
    exists = session.scalar(select(ChannelStream.stream_id).where(ChannelStream.stream_id == stream_id))
    if exists is None:
    ...
    tests = session.scalars(
        ...
        .order_by(StreamTest.completed_at.desc(), StreamTest.id.desc())
        .limit(limit)
    ).all()
